...
plt.xlabel('Cycles' if args.type == 'epoch' else 'episodes')
plt.ylabel('Reward per Cycle' if args.type == 'epoch' else 'reward per episode')
for idx, Y in enumerate(allY):
   ax.plot(Y, label=LABELS[idx])

plt.title('Instruction Following Learning Performance (onehot)')
